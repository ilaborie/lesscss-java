...
public class LessCompiler {
	...
	private Logger log = LoggerFactory.getLogger(LessCompiler.class);
	...
	private URL lessJs = LessCompiler.class.getClassLoader().getResource(
			"META-INF/less.js");
	private List<URL> customJs = Lists.newArrayList();
	...
	public void init() {
		...
		try {
			cx.evaluateReader(scope, new InputStreamReader(envJs
					.openConnection().getInputStream()), "env.rhino.js", 1,
					null);
			cx.evaluateReader(scope, new InputStreamReader(lessJs
					.openConnection().getInputStream()), "less.js", 1, null);

			for (URL url : customJs) {
				cx.evaluateReader(scope, new InputStreamReader(url
		...
		log.debug("Finished initialization of LESS compiler in {} ms.",
	...
	public String compile(String input) throws LessException {
		...
		try {
			...
			cx.evaluateString(scope, String.format(COMPILE_STRING, compress),
					"compile.js", 1, null);
			Object result = scope.get("result", scope);

			log.debug("Finished compilation of LESS source in {} ms.",
					(System.currentTimeMillis() - start));
	...
	public void compile(LessSource input, File output, boolean force)
		...
		if (force
			...
			Files.write(data, output, Charset.forName(encoding));
		}
	}
}
